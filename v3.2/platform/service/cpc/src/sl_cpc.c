 ...
 ******************************************************************************/
#if defined(SL_CATALOG_KERNEL_PRESENT)
#define LOCK_ENDPOINTS_LIST()            EFM_ASSERT(osMutexAcquire(endpoints_list_lock, osWaitForever) == osOK)
#define RELEASE_ENDPOINTS_LIST()         EFM_ASSERT(osMutexRelease(endpoints_list_lock) == osOK)
#define LOCK_ENDPOINT(ep)                EFM_ASSERT(ep->state != SL_CPC_STATE_CLOSED && osMutexAcquire(ep->lock, osWaitForever) == osOK)
#define RELEASE_ENDPOINT(ep)             EFM_ASSERT(ep->state != SL_CPC_STATE_CLOSED && osMutexRelease(ep->lock) == osOK)
#else
...
static sl_slist_node_t *closing;
...
static void add_closing(sl_cpc_endpoint_t *ep);

static bool free_closing_endpoint_if_empty(sl_cpc_endpoint_t *ep);
...
sl_status_t sl_cpc_close_endpoint(sl_cpc_endpoint_handle_t *endpoint_handle)
{
  ...
  if (!free_closing_endpoint_if_empty(ep)) {
    RELEASE_ENDPOINT(ep);
  }
...
void sli_cpc_drv_notify_tx_complete(sl_cpc_buffer_handle_t *buffer_handle)
{
  ...
  CORE_ENTER_ATOMIC();
  if ((buffer_handle->endpoint == NULL)                 // Endpoint already closed
    ...
    if (frame_type == SLI_CPC_HDLC_FRAME_TYPE_DATA) {
      SLI_CPC_DEBUG_TRACE_ENDPOINT_DATA_FRAME_TRANSMIT_COMPLETED(buffer_handle->endpoint);
      status = sl_sleeptimer_restart_timer(&buffer_handle->endpoint->re_transmit_timer,
                                           buffer_handle->endpoint->re_transmit_timeout,
      ...
      if (buffer_handle->endpoint->on_uframe_write_completed != NULL) {
        // Notify caller that it can free the tx buffer now
        buffer_handle->endpoint->on_uframe_write_completed(buffer_handle->endpoint->id, buffer_handle->data, buffer_handle->arg, SL_STATUS_OK);
      }
      ...
      // Increase tx window
      buffer_handle->endpoint->current_tx_window_space++;

      if (buffer_handle->endpoint->holding_list != NULL) {
        // Put data frames hold in the endpoint in the tx queue if space in transmit window

        while (buffer_handle->endpoint->holding_list != NULL
               && buffer_handle->endpoint->current_tx_window_space > 0) {
          sl_cpc_transmit_queue_item_t *qitem;
          uint8_t type;

          sl_slist_node_t *item_node = sl_slist_pop(&buffer_handle->endpoint->holding_list);
          ...
          if ((type == SLI_CPC_HDLC_FRAME_TYPE_UNNUMBERED)
              || (buffer_handle->endpoint->frames_count_re_transmit_queue == 0)) {
            // If next frame in holding list is an unnumbered or if no frame to retx
            // Transmit uframe or iframe
            sl_slist_push_back(&transmit_queue, item_node);
            buffer_handle->endpoint->current_tx_window_space--;
          } else {
            // if next is iframe; wait ACK before transmitting
            sl_slist_push(&buffer_handle->endpoint->holding_list, item_node);
            break;
          }
        }
      } else if (buffer_handle->endpoint->state == SL_CPC_STATE_CLOSING) {
        close = true;
      }
    } else {
      SLI_CPC_DEBUG_TRACE_ENDPOINT_SUPERVISORY_FRAME_TRANSMIT_COMPLETED(buffer_handle->endpoint);
    }
  }
  ...
  if (close) {
    add_closing(buffer_handle->endpoint);
  }
}
                                 ...
                                 uint8_t tx_window_size)
{
  ...
  ep->node.node = NULL;
  ep->re_transmit_timeout = sl_sleeptimer_ms_to_tick(SLI_CPC_MAX_RE_TRANSMIT_TIMEOUT_MS);
                         ...
                         void *on_write_completed_arg)
{
  ...
  if (iframe) {
    ...
    frame_handle->control = sli_cpc_hdlc_create_control_data(ep->seq, ep->ack);
  ...
  if (ep->current_tx_window_space > 0) {
    ...
    CORE_ATOMIC_SECTION(sl_slist_push_back(&transmit_queue, &item->node); );
    ...
    sl_slist_push_back(&ep->holding_list, &item->node);
  }
...
static void decode_packet(void)
{
  ...
  if (endpoint != NULL) {
    ...
    if (rx_handle->reason != SL_CPC_REJECT_NO_ERROR) {
      ...
      receive_iframe(endpoint, rx_handle, address, seq);
    ...
    if (endpoint->state == SL_CPC_STATE_CLOSING) {
      if (!free_closing_endpoint_if_empty(endpoint)) {
                        ...
                        uint8_t ack)
{
  ...
  while (endpoint->holding_list != NULL
    ...
    CORE_ATOMIC_SECTION(sl_slist_push_back(&transmit_queue, item); );
    endpoint->current_tx_window_space--;
  }
                           ...
                           uint8_t seq)
{
  ...
  if (seq == endpoint->ack) {
    if (sli_cpc_push_back_rx_data_in_receive_queue(rx_handle, &endpoint->iframe_receive_queue, rx_buffer_payload_len) != SL_STATUS_OK) {
                           ...
                           uint16_t data_length)
{
  ...
  if ((fatal_error)
    ...
    // Free is completed in free_closing_endpoint_if_empty
  }
}
...
static void receive_uframe(sl_cpc_endpoint_t *endpoint,
                           ...
                           uint16_t data_length)
{
  uint16_t payload_len = data_length - SLI_CPC_HDLC_FCS_SIZE;
  uint8_t type;
  uint16_t fcs;
  ...
  if (!sli_cpc_validate_crc_sw(rx_handle->data, payload_len, fcs)) {
  ...
  if ((type == SLI_CPC_HDLC_CONTROL_UNNUMBERED_TYPE_INFORMATION)
    ...
    if (endpoint->poll_final.on_final != NULL) {
      endpoint->poll_final.on_final(endpoint->id, endpoint->poll_final.on_fnct_arg, rx_handle->data, rx_handle->data_length);
    }
#endif
#if defined(SL_CPC_ON_POLL_PRESENT)
    if (endpoint->poll_final.on_poll != NULL) {
      ...
      if (reply_data != NULL && reply_data_lenght > 0) {
        sl_status_t status = write(endpoint, reply_data, reply_data_lenght, SL_CPC_FLAG_UNNUMBERED_POLL, on_write_completed_arg);
        if ((status != SL_STATUS_OK)
...
static sl_status_t re_transmit_frame(sl_cpc_endpoint_t * endpoint)
{
  ...
  //Put frame in Tx Q so that it can be transmitted by CPC Core later
  CORE_ATOMIC_SECTION(sl_slist_push(&transmit_queue, &item->node); );
...
static sl_status_t process_tx_queue(void)
{
  ...
  CORE_ATOMIC_SECTION(node = sl_slist_pop(&transmit_queue); );
...
static void process_closing(void)
{
  if (closing != NULL) {
    sl_slist_node_t *node;
    CORE_ATOMIC_SECTION(node = sl_slist_pop(&closing); );
    do {
      sl_cpc_endpoint_t *endpoint = SL_SLIST_ENTRY(node, sl_cpc_endpoint_t, node_closing);
      bool closed;
      LOCK_ENDPOINT(endpoint);
      closed = free_closing_endpoint_if_empty(endpoint);
      CORE_ATOMIC_SECTION(node = sl_slist_pop(&closing); );
      if (!closed) {
        RELEASE_ENDPOINT(endpoint);
      }
...
static void add_closing(sl_cpc_endpoint_t * ep)
{
  CORE_ATOMIC_SECTION(sl_slist_push(&closing, &ep->node_closing); );
  sli_cpc_signal_event(SL_CPC_SIGNAL_CLOSING);
}

/***************************************************************************//**
 * Try to free endpoint in closing state
 ******************************************************************************/
static bool free_closing_endpoint_if_empty(sl_cpc_endpoint_t *ep)
{
  bool wait = false;
  ...
  // This function must not be called if the endpoint is not in the closing state
  EFM_ASSERT(ep->state == SL_CPC_STATE_CLOSING);
...
static void clean_tx_queues(sl_cpc_endpoint_t * endpoint)
{
  ...
  while (node != NULL) {
    ...
    if (queue_item->handle->endpoint == endpoint) {
      ...
      sl_slist_remove(&transmit_queue, &queue_item->node);
  ...
  while (node != NULL) {
    ...
    sl_slist_remove(&endpoint->holding_list, &queue_item->node);
    queue_item->handle->data = NULL;
    sli_cpc_drop_buffer_handle(queue_item->handle);
    sli_cpc_free_transmit_queue_item(queue_item);
  }
  ...
  while (node != NULL) {
    ...
    node = node->node;
    sl_slist_remove(&endpoint->re_transmit_queue, &queue_item->node);
    if (queue_item->handle->drv_ref == false) {
