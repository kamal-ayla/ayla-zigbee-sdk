...
sl_status_t sli_cpc_drv_init(void)
{
  ...
  for (buf_cnt = 0; buf_cnt < SL_CPC_DRV_UART_RX_QUEUE_SIZE; buf_cnt++) {
    ...
    EFM_ASSERT(sli_cpc_get_rx_buffer(&entry->buf_handle) == SL_STATUS_OK);
    sl_slist_push(&rx_free_list_head, &entry->node);
  }
  ...
  while (dma_desc_cnt < RX_DMA_DESCRIPTOR_QTY) {
    ...
    *current_desc = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_SINGLE_P2M_BYTE(&(SL_CPC_DRV_UART_PERIPHERAL->RXDATA), buffer_ptr, DMA_MAX_XFER_LEN);
...
sl_status_t sli_cpc_drv_read_data(sl_cpc_buffer_handle_t **buffer_handle, uint16_t *payload_rx_len)
{
  ...
  CORE_ENTER_ATOMIC();
  sli_buf_entry_t *entry = (sli_buf_entry_t *)sl_slist_pop(&rx_pending_list_head);
  if (entry == NULL) {
    CORE_EXIT_ATOMIC();
  ...
  *buffer_handle = entry->buf_handle;
  *payload_rx_len = SLI_CPC_RX_DATA_MAX_LENGTH;

  CORE_ENTER_ATOMIC();
  status = sli_cpc_get_rx_buffer(&entry->buf_handle);
...
sl_status_t sli_cpc_drv_transmit_data(sl_cpc_buffer_handle_t *buffer_handle, uint16_t payload_tx_len)
{
  ...
  entry->buf_handle = buffer_handle;
  entry->payload_len = payload_tx_len;

  CORE_ENTER_ATOMIC();
  sl_slist_push_back(&tx_submitted_list_head, &entry->node);
...
static sl_status_t prepare_next_tx(void)
{
  ...
  if (entry->payload_len <= DMADRV_MAX_XFER_COUNT && entry->payload_len != 0u) {
    tx_descriptor[0u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_LINKREL_M2P_BYTE(entry->buf_handle->hdlc_header, &(SL_CPC_DRV_UART_PERIPHERAL->TXDATA), SLI_CPC_HDLC_HEADER_RAW_SIZE, 1);
    tx_descriptor[1u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_LINKREL_M2P_BYTE(entry->buf_handle->data, &(SL_CPC_DRV_UART_PERIPHERAL->TXDATA), entry->payload_len, 1u);
    tx_descriptor[2u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_SINGLE_M2P_BYTE(entry->buf_handle->fcs, &(SL_CPC_DRV_UART_PERIPHERAL->TXDATA), 2u);
    ...
    tx_descriptor[0u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_LINKREL_M2P_BYTE(entry->buf_handle->hdlc_header, &(SL_CPC_DRV_UART_PERIPHERAL->TXDATA), SLI_CPC_HDLC_HEADER_RAW_SIZE, 1);
    tx_descriptor[1u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_LINKREL_M2P_BYTE(entry->buf_handle->data, &(SL_CPC_DRV_UART_PERIPHERAL->TXDATA), DMADRV_MAX_XFER_COUNT, 1u);
    tx_descriptor[2u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_LINKREL_M2P_BYTE(&((uint8_t *)entry->buf_handle->data)[DMADRV_MAX_XFER_COUNT], &(SL_CPC_DRV_UART_PERIPHERAL->TXDATA), (entry->payload_len - DMADRV_MAX_XFER_COUNT), 1u);
    tx_descriptor[3u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_SINGLE_M2P_BYTE(entry->buf_handle->fcs, &(SL_CPC_DRV_UART_PERIPHERAL->TXDATA), 2u);
    tx_descriptor[0u].xfer.doneIfs = 0u;
    tx_descriptor[1u].xfer.doneIfs = 0u;
    tx_descriptor[2u].xfer.doneIfs = 0u;
    tx_descriptor[3u].xfer.doneIfs = 1u;
  } else if (entry->payload_len == 0u) {
    tx_descriptor[0u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_SINGLE_M2P_BYTE(entry->buf_handle->hdlc_header, &(SL_CPC_DRV_UART_PERIPHERAL->TXDATA), SLI_CPC_HDLC_HEADER_RAW_SIZE);
  ...
  if (flag & USART_IF_RXDATAV) {
    ...
    if (rx_synch_byte_cnt == SLI_CPC_HDLC_HEADER_RAW_SIZE) {
      ...
      if (current_rx_entry == NULL) {
        current_rx_entry = (sli_buf_entry_t *)sl_slist_pop(&rx_free_list_head);
        if (current_rx_entry == NULL) {
      ...
      ((uint8_t *)current_rx_entry->buf_handle->hdlc_header)[1] = rx_synch_header[1];
      ((uint8_t *)current_rx_entry->buf_handle->hdlc_header)[2] = rx_synch_header[2];
      ((uint8_t *)current_rx_entry->buf_handle->hdlc_header)[3] = rx_synch_header[3];
      ((uint8_t *)current_rx_entry->buf_handle->hdlc_header)[4] = rx_synch_header[4];
      current_rx_entry->buf_handle->data_length = payload_len;

      next_rx_payload_len = payload_len;

      if (payload_len == 0) {
        // Push rx_entry to pending list
        sl_slist_push_back(&rx_pending_list_head, &current_rx_entry->node);
        // Notify core
...
void SL_CPC_ISR_TX_HANDLER(SL_CPC_DRV_UART_PERIPHERAL_NO)(void)
{
  ...
  if (flag & USART_IF_TXC) {
    ...
    CORE_ENTER_ATOMIC();
    entry = (sli_buf_entry_t *)sl_slist_pop(&tx_submitted_list_head);
    CORE_EXIT_ATOMIC();

    // Notify core
    sli_cpc_drv_notify_tx_complete(entry->buf_handle);
                            ...
                            void *userParam)
{
  ...
  rx_descriptor_head = (LDMA_Descriptor_t *)(uint32_t)(rx_descriptor_head->xfer.linkAddr << 2);
    ...
    if (header_expected_next) {
      ...
      if (current_rx_buf_tot_len == 0) {
        ...
        if ((next_rx_payload_len > 0)
          ...
          // Here we could improve things by also updating the length of the
          // DMA descriptor after the next one to SLI_CPC_HDLC_HEADER_RAW_SIZE.
          ...
          if (!update_success) {
            ...
            if (!update_success) {
              // Not supported in this proof of concept. Probably an acceptable limitation.
              EFM_ASSERT(update_success);
            }
          }
      ...
      // Copy useful fields of header. Unfortunately with this method the header must always be copied
      // Copy only useful bytes
      ((uint8_t *)current_rx_entry->buf_handle->hdlc_header)[1] = current_rx_buffer[1];
      ((uint8_t *)current_rx_entry->buf_handle->hdlc_header)[2] = current_rx_buffer[2];
      ((uint8_t *)current_rx_entry->buf_handle->hdlc_header)[3] = current_rx_buffer[3];
      ((uint8_t *)current_rx_entry->buf_handle->hdlc_header)[4] = current_rx_buffer[4];
      current_rx_entry->buf_handle->data_length = next_rx_payload_len;
      ...
      current_rx_entry->buf_handle->data = rx_buffer;
      current_rx_entry->buf_handle->data_length = next_rx_payload_len;
      ...
      // Notify core
      // Normally we should not push the buffer to the core here unless we are
      // 100% sure it doesn't contain something else like a header or another
      // payload. But with the current implementation/limitation, it is fine.
      sl_slist_push_back(&rx_pending_list_head, &current_rx_entry->node);
      sli_cpc_drv_notify_rx_data();
  ...
  if (rx_buf_free) {
    ...
    LDMA_Descriptor_t *desc = (LDMA_Descriptor_t *)(completed_desc->xfer.linkAddr << 2u);
    ...
    for (uint16_t i = 1u; i < nb_desc_free; i++) {
      ...
      desc = (LDMA_Descriptor_t *)(temp_desc->xfer.linkAddr << 2u);
...
static bool update_current_rx_dma_length(uint16_t new_length)
{
  ...
  already_recvd_cnt = ((ctrl & _LDMA_CH_CTRL_XFERCNT_MASK) >> _LDMA_CH_CTRL_XFERCNT_SHIFT) + 1u;

  if (already_recvd_cnt != DMA_MAX_XFER_LEN) {
...
static bool update_current_rx_dma_large_payload_length(uint16_t new_length)
{
  ...
  while (desc_cnt < nb_desc) {
    ...
    desc = (LDMA_Descriptor_t *)(desc->xfer.linkAddr << 2u);
    desc_cnt++;
  }
...
static void push_back_new_rx_dma_desc(LDMA_Descriptor_t *new_dma_desc,
                                      void *buffer)
{
  *new_dma_desc = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_SINGLE_P2M_BYTE(&(SL_CPC_DRV_UART_PERIPHERAL->RXDATA), buffer, DMA_MAX_XFER_LEN);
...
static void notify_core_error(sl_cpc_reject_reason_t reason)
{
  current_rx_entry->buf_handle->reason = reason;
