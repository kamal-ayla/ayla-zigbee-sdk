...
#if (SLI_CPC_RX_DATA_MAX_LENGTH <= DMA_MAX_XFER_LEN)
    rx_descriptor[0u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_LINKREL_P2M_BYTE(&(SL_CPC_DRV_SPI_PERIPHERAL->RXDATA), current_rx_entry->buf_handle->hdlc_header, SLI_CPC_HDLC_HEADER_RAW_SIZE, 1);
    rx_descriptor[1u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_SINGLE_P2M_BYTE(&(SL_CPC_DRV_SPI_PERIPHERAL->RXDATA), current_rx_entry->buf_handle->data, SLI_CPC_RX_DATA_MAX_LENGTH);
    rx_descriptor[0].xfer.doneIfs = 0u;
    rx_descriptor[1].xfer.doneIfs = 0u;
#else
    rx_descriptor[0u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_LINKREL_P2M_BYTE(&(SL_CPC_DRV_SPI_PERIPHERAL->RXDATA), current_rx_entry->buf_handle->hdlc_header, SLI_CPC_HDLC_HEADER_RAW_SIZE, 1);
    rx_descriptor[1u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_LINKREL_P2M_BYTE(&(SL_CPC_DRV_SPI_PERIPHERAL->RXDATA), current_rx_entry->buf_handle->data, DMADRV_MAX_XFER_COUNT, 1);
    rx_descriptor[2u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_SINGLE_P2M_BYTE(&(SL_CPC_DRV_SPI_PERIPHERAL->RXDATA), &((uint8_t *)current_rx_entry->buf_handle->data)[DMADRV_MAX_XFER_COUNT], SLI_CPC_RX_DATA_MAX_LENGTH - DMADRV_MAX_XFER_COUNT);
    rx_descriptor[0].xfer.doneIfs = 0u;
    rx_descriptor[1].xfer.doneIfs = 0u;
...
static sl_status_t prepare_next_tx(void)
{
  ...
  if (entry->payload_len <= DMADRV_MAX_XFER_COUNT && entry->payload_len != 0u) {
    tx_descriptor[0u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_LINKREL_M2P_BYTE(entry->buf_handle->hdlc_header, &(SL_CPC_DRV_SPI_PERIPHERAL->TXDATA), SLI_CPC_HDLC_HEADER_RAW_SIZE, 1);
    tx_descriptor[1u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_LINKREL_M2P_BYTE(entry->buf_handle->data, &(SL_CPC_DRV_SPI_PERIPHERAL->TXDATA), entry->payload_len, 1u);
    tx_descriptor[2u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_SINGLE_M2P_BYTE(entry->buf_handle->fcs, &(SL_CPC_DRV_SPI_PERIPHERAL->TXDATA), 2u);
    ...
    tx_descriptor[0u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_LINKREL_M2P_BYTE(entry->buf_handle->hdlc_header, &(SL_CPC_DRV_SPI_PERIPHERAL->TXDATA), SLI_CPC_HDLC_HEADER_RAW_SIZE, 1);
    tx_descriptor[1u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_LINKREL_M2P_BYTE(entry->buf_handle->data, &(SL_CPC_DRV_SPI_PERIPHERAL->TXDATA), DMADRV_MAX_XFER_COUNT, 1u);
    tx_descriptor[2u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_LINKREL_M2P_BYTE(&((uint8_t *)entry->buf_handle->data)[DMADRV_MAX_XFER_COUNT], &(SL_CPC_DRV_SPI_PERIPHERAL->TXDATA), (entry->payload_len - DMADRV_MAX_XFER_COUNT), 1u);
    tx_descriptor[3u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_SINGLE_M2P_BYTE(entry->buf_handle->fcs, &(SL_CPC_DRV_SPI_PERIPHERAL->TXDATA), 2u);
    tx_descriptor[0u].xfer.doneIfs = 0u;
    tx_descriptor[1u].xfer.doneIfs = 0u;
    tx_descriptor[2u].xfer.doneIfs = 0u;
    tx_descriptor[3u].xfer.doneIfs = 1u;
  } else if (entry->payload_len == 0u) {
    tx_descriptor[0u] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_SINGLE_M2P_BYTE(entry->buf_handle->hdlc_header, &(SL_CPC_DRV_SPI_PERIPHERAL->TXDATA), 7u);
  ...
  if (current_rx_entry) {
    ...
    sl_slist_push(&rx_free_list_head, &current_rx_entry->node);
    current_rx_entry = NULL;
  }
