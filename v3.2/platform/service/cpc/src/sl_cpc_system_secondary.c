...
#ifdef SLI_CPC_ENDPOINT_SECURITY
#include "sl_cpc_security_secondary.h"
#endif

#define CPC_PACKED_ENDPOINT_PRESENT 0   // Not yet implemented

#ifndef SLI_CPC_SYSTEM_COMMAND_BUFFER_COUNT
#define SLI_CPC_SYSTEM_COMMAND_BUFFER_COUNT 2
#endif

#define CAPABILITIES                                                     \
  (SLI_CPC_ENDPOINT_SECURITY * CPC_CAPABILITIES_SECURITY_ENDPOINT_MASK   \
   + CPC_PACKED_ENDPOINT_PRESENT * CPC_CAPABILITIES_PACKED_ENDPOINT_MASK \
   + SLI_CPC_ENDPOINT_GPIO * CPC_CAPABILITIES_GPIO_ENDPOINT_MASK)
...
/***************************************************************************//**
 * System endpoint command buffer pool.
 ******************************************************************************/
#define SLI_CPC_SYSTEM_COMMAND_BUFFER_SIZE    (sizeof(sl_cpc_system_cmd_t) + sizeof(sl_cpc_system_property_cmd_t) \
                                               + SL_CPC_ENDPOINT_MAX_COUNT * sizeof(sl_cpc_endpoint_state_t) / 2)

static sli_mem_pool_handle_t mempool_system_command_buffer;
SLI_MEM_POOL_DECLARE_BUFFER(mempool_system_command_buffer,
...
sl_status_t sli_cpc_system_init(void)
{
  ...
  sli_mem_pool_create(&mempool_system_command_buffer,
                      SLI_CPC_SYSTEM_COMMAND_BUFFER_SIZE,
                      SLI_CPC_SYSTEM_COMMAND_BUFFER_COUNT,
                      mempool_system_command_buffer_buffer,
                      sizeof(mempool_system_command_buffer_buffer));

  status = sli_cpc_open_service_endpoint(&system_ep, SL_CPC_ENDPOINT_SYSTEM,
                                         SL_CPC_OPEN_ENDPOINT_FLAG_IFRAME_DISABLE   // Auto-drop iframe
                                         | SL_CPC_OPEN_ENDPOINT_FLAG_UFRAME_ENABLE, // Enable uframe for poll-final
                                         1);
                               ...
                               sl_status_t status)
{
  ...
  sli_mem_pool_free(&mempool_system_command_buffer, buffer);
...
static void send_reset_reason(void)
{
  ...
  tx_command = sli_mem_pool_alloc(&mempool_system_command_buffer);
  if (tx_command == NULL) {
    return;
  }
  ...
  if (reset_cause & rmuResetAVDD) {
    *tx_prop_last_status = STATUS_RESET_FAULT;
  } else if (reset_cause & rmuResetIOVDD0) {
    *tx_prop_last_status = STATUS_RESET_FAULT;
  } else if (reset_cause & rmuResetDecouple) {
    *tx_prop_last_status = STATUS_RESET_FAULT;
  } else if ((reset_cause & rmuResetWdog0)
             || (reset_cause & rmuResetWdog1)) {
    *tx_prop_last_status = STATUS_RESET_WATCHDOG;
  } else if ((reset_cause & rmuResetCoreLockup)
             || (reset_cause & rmuResetSELockup)) {
    *tx_prop_last_status = STATUS_RESET_CRASH;
  } else if ((reset_cause & rmuResetSys)
             || (reset_cause & rmuResetSESys)) {
    *tx_prop_last_status = STATUS_RESET_SOFTWARE;
  } else {
    *tx_prop_last_status = STATUS_RESET_UNKNOWN;
  }
...
static void on_property_get_protocol_version(sl_cpc_system_cmd_t *tx_command)
{
  ...
  version[0] = SL_CPC_VERSION_MAJOR;
  version[1] = SL_CPC_VERSION_MINOR;

  tx_command->header.length = sizeof(sl_cpc_property_id_t) + (2 * sizeof(uint32_t));
}
  ...
  *reply_data_lenght = sizeof(sl_cpc_system_cmd_header_t) + reset->header.length;
  ...
  if (*reset_status == STATUS_OK) {
    ...
    *on_write_complete_arg = (void*)0xDEADBEEF;
    ...
    // The reset won't occurre, dont tell the callback to reset
