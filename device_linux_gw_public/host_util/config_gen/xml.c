...
static int xml_putc(struct xml_state *state, unsigned char byte)
{
	if (state->textp >= &state->text_buf[XML_MAX_TEXT - 1]) {
		log_err("xml_putc: text limit exceeded");
		return -1;
	}
	*state->textp++ = byte;
	return 0;
...
int xml_parse(struct xml_state *state, void *buf, size_t len)
{
	...
	for (len_used = 0; len_used < len; len_used++) {
			...
			if (byte == ' ') {
				...
				*/
				if (state->depth >= XML_MAX_DEPTH) {
					log_err("tag depth limit exceeded. "
					    "tag '%s'", name);
					goto error;
				}
				state->depth++;
		...
		case XS_DONE:
			...
			log_err("extra text '%c' after done",
			    byte);
			goto error;
		case XS_IDLE:
		...
		case XS_ERROR:
			goto error;
		default:
			log_err("unknown state %u", state->state);
			goto error;
		}
	}
out:
