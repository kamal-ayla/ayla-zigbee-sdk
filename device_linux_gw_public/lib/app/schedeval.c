 ...
 */
static int sched_set_sched_value(void *dest_val, u8 dest_len,
	struct ayla_tlv *tlv)
{
	u64 value;

	if (endian_put_ntoh_u64(&value, TLV_VAL(tlv), tlv->len) < 0) {
		log_warn("byte swap length invalid: %hhu", tlv->len);
		return -1;
	}
	switch (dest_len) {
	case sizeof(u8):
	...
	case sizeof(u64):
		*((u64 *)dest_val) = value;
		break;
	default:
		log_warn("dest length invalid: %hhu", dest_len);
		return -1;
	}
	return 0;
}
					...
					u32 duration)
{
	...
	tmp1 = clock_local(&daylight_ayla.change);
 ...
 * is set to the beginning of the interval and range_end is the end.
...
set_start_time_each_day:
		...
		if (duration_no_interval) {
			...
			if (!schedule->is_utc && daylight_ayla.valid &&
			    !daylight_ayla.active) {
...
local_to_utc:
	...
	if (toplevel && !schedule->is_utc) {
		if (daylight_ayla.valid && !daylight_ayla.active) {
		...
		if (!schedule->interval && schedule->duration) {
			...
			*range_end = *range_start +
			...
			if (in_range && time > *range_end) {
				...
				time = daylight_ayla.change + DAYLIGHT_OFFSET;
...
u32 sched_evaluate(struct sched_prop *schedtlv, u32 time)
{
	...
	if (atlv->type != ATLV_VERSION ||
	    endian_put_ntoh_u64(&value, TLV_VAL(atlv), atlv->len) < 0 ||
	    value > SCHED_LIB_VERSION) {
		log_warn("bad sched ver");
		return 0;
	}
	...
	while (consumed < schedtlv->len) {
		...
		case ATLV_INRANGE:
			...
			if (!time || (!sched.is_utc && !timezone_ayla.valid)) {
				log_warn("curr time not known");
				return 0;
			}
			if (range_calculated) {
		...
		case ATLV_SETPROP:
			if ((type == ATLV_ATSTART && time == range_start) ||
			    ...
			    time < range_end)) {
				sched_fire_schedule(schedtlv, atlv, atlv->len);
			}
			break;
		default:
			log_warn("unknown sched tlv = %x", atlv->type);
			return 0;
		}
		if (err) {
			log_warn("sched value err");
			return 0;
		}
		consumed += atlv->len + sizeof(struct ayla_tlv);
	}
