	...
	void (*msg_handler)(DBusMessage *, void *), void *arg)
{
	...
	log_debug("filter->type %d, filter->sender %s, filter->interface %s,"
	    " filter->member %s, filter->path %s, filter->destination %s",
	    filter->type, filter->sender, filter->interface,
	    filter->member, filter->path, filter->destination);
...
static void dbus_client_obj_path_cleanup(
	struct dbus_client_obj_handler *handler)
{
	if (!handler) {
		return;
	}
	free((void *)handler->handler.filter.sender);
	free((void *)handler->handler.filter.interface);
	free((void *)handler->handler.filter.member);
	free((void *)handler->handler.filter.path);
	free((void *)handler->handler.filter.destination);
	free((void *)handler);
}
	...
	void (*msg_handler)(DBusMessage *, void *), void *arg)
{
	...
	if (!success) {
		dbus_client_obj_path_cleanup(handler);
		return -1;
	}
	return 0;
}
...
static void dbus_client_new_path_free(DBusConnection *connection,
	void *user_data)
{
	struct dbus_client_obj_handler *handler =
	    (struct dbus_client_obj_handler *)user_data;

	dbus_client_msg_unsubscribe(&handler->handler.filter);
	...
	void (*msg_handler)(DBusMessage *, void *), void *arg)
{
	struct dbus_client_obj_handler *handler;
	struct dbus_client_msg_filter filter = {
		.type = DBUS_MSG_TYPE_METHOD_CALL,
		.interface = interface,
		.path = path,
	};
	DBusError err;
	...
	dbus_client_msg_handler_init(&handler->handler, msg_handler, arg,
	    &filter);
	handler->vtable.unregister_function = dbus_client_new_path_free;
	handler->vtable.message_function = dbus_client_obj_msg_handler;
	/* Register the new path on the bus */
	dbus_error_init(&err);
	success = dbus_connection_try_register_object_path(state.conn, path,
