...
/*
 * Bluetooth Gateway Demo
 *
 * This gateway application hooks into a generic node management interface
 * and a simple Bluetooth gateway implementation to demonstrate good practices
 ...
 * In order to achieve the most flexible demo platform, the Bluetooth interface
 * layer communicates with the BlueZ user-space agent (bluetoothd) via its
 * D-Bus interface.
 *
 */
...
const char *appd_version = "bt_gatewayd " BUILD_VERSION_LABEL;
const char *appd_template_version = "bluetooth_gateway_demo_v1.2";
 ...
 */
static void appd_node_network_callback_init(void)
{
	struct node_network_callbacks network_callbacks = {
		.node_query_info = bt_query_info_handler,
		.node_configure = bt_configure_handler,
		.node_prop_set = bt_prop_set_handler,
		.node_leave = bt_leave_handler
	};
...
int appd_init(void)
{
	...
	/* Load property table */
	rc = prop_add(appd_gw_prop_table, ARRAY_LEN(appd_gw_prop_table));
	if (rc < 0) {
		log_err("prop_add returned error");
		return -1;
	}
	 ...
	 */
	rc = node_mgmt_init(app_get_timers());
	if (rc < 0) {
		log_err("node_mgmt_init returned error");
		return -1;
	}
