			...
			uint8_t data_type, uint8_t *data)
{
	if (attr_id == ZCL_LOCAL_TEMPERATURE_ATTRIBUTE_ID) {
		if (data_type == ZCL_INT16S_ATTRIBUTE_TYPE) {
			double celsius, fahrenheit, v;
			celsius = (*(int16_t *)data / (double)100.00);
			v = (celsius * 9 / 5) + 32;
			fahrenheit = ((int)(v*100.00))/(double)100.00;
			log_debug("received temperature measurement for thermostat------- %f Fahrenheit",fahrenheit);
			log_debug("received temperature measurement for thermostat------- %f Celsius",celsius);
			appd_update_decimal_prop(source, ZB_LOCAL_TEMPERATURE,fahrenheit);
		...
		if (data_type == ZCL_INT16S_ATTRIBUTE_TYPE) {
			...
			celsius = (*(int16_t *)data / 100);
			float v = ((float)celsius * 9 / 5) + 32;
			fahrenheit = (int)(v+0.5);
			log_debug("received cooling setpoint for thermostat------- %d data value",(*(int16_t *)data / 100));
			...
			appd_update_int_prop(source, ZB_COOLING_SETPOINT,fahrenheit);
		...
		if (data_type == ZCL_INT16S_ATTRIBUTE_TYPE) {
			...
			float v = ((float)celsius * 9 / 5) + 32;
			fahrenheit = (int)(v+0.5);
			log_debug("received heating setpoint for thermostat------- %d data value",(*(int16_t *)data / 100));
			...
			appd_update_int_prop(source, ZB_HEATING_SETPOINT,fahrenheit);
		...
		case ZCL_REPORT_ATTRIBUTES_COMMAND_ID:
			...
			if (attr->attr_id == ZCL_ON_OFF_ATTRIBUTE_ID) {
				...
				appd_update_node_prop(source,
				    ZB_IS_ON_OFF_PROP_NAME, &(attr->value));
				/* appd_ieee_query_handler(source); */
			}
			break;
		case ZCL_READ_ATTRIBUTES_RESPONSE_COMMAND_ID:
			log_debug("msg = %x", *(msg + 4));
			appd_update_node_prop(source,
				ZB_IS_ON_OFF_PROP_NAME, msg+4);
			break;
...
/*
 * Handle door lock cluster message
 */
static bool zbc_door_lock_cluster_msg_handle(uint16_t source,
			uint8_t frame_type, uint8_t cmd_id, uint8_t *msg,
			uint16_t msgLen)
{
	uint8_t *pin;
	...
	if (frame_type == ZCL_CLUSTER_SPECIFIC_COMMAND) {
		switch (cmd_id) {
		case ZCL_GET_PIN_COMMAND_ID:
			/* msg is likely of tlv standard */
			pinLen = msgLen - 5;
			log_debug("Received get PIN response from"
			    " source=0x%04X", source);
			pin = (uint8_t *) malloc(sizeof(uint8_t)*(pinLen+1));
			if (pin == NULL) {
				log_debug("error allocating memory");
				return false;
			}
			strncpy((char *)pin, (char *)(msg + 5), pinLen);
			*(pin + pinLen) = (uint8_t)'\0';
			appd_update_node_prop(source, ZB_PIN_PROP_NAME, pin);
			free(pin);
			break;
		default:
			log_debug("Received invalid door command 0x%02X"
			    " from source=0x%04X", cmd_id, source);
			break;
		}
		...
		case ZCL_CONFIGURE_REPORTING_RESPONSE_COMMAND_ID:
			...
			break;
		case ZCL_REPORT_ATTRIBUTES_COMMAND_ID:
			log_debug("received msg with length = %d", msgLen);
			*(msg + 3) -= 2;
			*(msg + 3) *= -1;
			appd_update_node_prop(source, ZB_IS_LOCKED_PROP_NAME,
				msg+3);
			break;
		case ZCL_READ_ATTRIBUTES_RESPONSE_COMMAND_ID:
			log_debug("received msg with length = %d", msgLen);
...
/*
 * Handle humidity measurement cluster message
 */
static bool zbc_humi_measurement_cluster_msg_handle(uint16_t source,
			uint8_t frame_type, uint8_t cmd_id, uint8_t *msg,
			uint16_t msgLen)
{
	if (frame_type == ZCL_GLOBAL_COMMAND) {
...
/*
 * Handle temperature measurement cluster message
 */
static bool zbc_temp_measurement_cluster_msg_handle(uint16_t source,
			uint8_t frame_type, uint8_t cmd_id, uint8_t *msg,
			uint16_t msgLen)
{
	double celsius, fahrenheit, v;
	if (frame_type == ZCL_GLOBAL_COMMAND) {
		switch (cmd_id) {
		case ZCL_REPORT_ATTRIBUTES_COMMAND_ID:
			log_debug("received temperature measurement ZCL_REPORT_ATTRIBUTES_COMMAND_ID"
				" attribute");
			celsius = (*(int16_t *)(msg + 3) / (double)100.00);
			v = (celsius * 9 / 5) + 32;
			fahrenheit = ((int)(v*100.00))/(double)100.00;
			log_debug("received temperature measurement ------- %f",fahrenheit);
			appd_update_decimal_prop(source, ZB_LOCAL_TEMPERATURE,fahrenheit);
			//appd_update_decimal_prop(source, ZB_LOCAL_TEMPERATURE,
			    //(*(int16_t *)(msg + 3) / (double)100.00));
			break;
		case ZCL_READ_ATTRIBUTES_RESPONSE_COMMAND_ID:
			log_debug("received temperature measurement ZCL_READ_ATTRIBUTES_RESPONSE_COMMAND_ID"
				" attribute");
			celsius = (*(int16_t *)(msg + 4) / (double)100.00);
			v = (celsius * 9 / 5) + 32;
			fahrenheit = ((int)(v*100.00))/(double)100.00;;
			log_debug("received temperature measurement ------- %f",fahrenheit);
			appd_update_decimal_prop(source, ZB_LOCAL_TEMPERATURE,fahrenheit);
			//appd_update_decimal_prop(source, ZB_LOCAL_TEMPERATURE,
			    //(*(int16_t *)(msg + 4) / (double)100.00));
			//appd_update_decimal_prop(source, ZB_LOCAL_TEMPERATURE,
			    //(*(int16_t *)(msg + 3) / (double)100.00));
			break;
		default:
...
static bool zbc_illum_measurement_cluster_msg_handle(uint16_t source,
			uint8_t frame_type, uint8_t cmd_id, uint8_t *msg,
			uint16_t msgLen)
{
	if (frame_type == ZCL_GLOBAL_COMMAND) {
		switch (cmd_id) {
		case ZCL_REPORT_ATTRIBUTES_COMMAND_ID:
			log_info("received report illumination attribute");
			appd_update_int_prop(source, ZB_LIGHT_PROP_NAME,
				*(short *)(msg + 3));
			break;
		default:
			log_debug("Received invalid global response 0x%02X"
		...
		log_debug("received invalid message frame type");
	}
	return true;
}
				...
				uint8_t *message, uint16_t msgLen)
{
	...
	case ZCL_DOOR_LOCK_CLUSTER_ID:
		ret = zbc_door_lock_cluster_msg_handle(source,
		    zcl->ctrl.frame_type, zcl->cmd_id, zcl->msg, msgLen - 3);
		break;
	case ZCL_TEMP_MEASUREMENT_CLUSTER_ID:
		ret = zbc_temp_measurement_cluster_msg_handle(source,
		    zcl->ctrl.frame_type, zcl->cmd_id, zcl->msg, msgLen);
		break;
	case ZCL_RELATIVE_HUMIDITY_MEASUREMENT_CLUSTER_ID:
		ret = zbc_humi_measurement_cluster_msg_handle(source,
		    zcl->ctrl.frame_type, zcl->cmd_id, zcl->msg, msgLen);
		break;
