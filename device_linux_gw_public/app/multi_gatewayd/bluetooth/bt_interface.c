...
/*
 * Save bluetooth node info
 */
json_t *bt_conf_save_handler(const struct node *node)
{
	return NULL;
}

/*
 * Restore bluetooth node info
 */
int bt_conf_loaded_handler(struct node *node, json_t *net_state_obj)
{
	return 0;
}
	...
	enum bt_dbus_interface type, void *data)
{
	DBusMessage *msg;
	const char *path = BT_DBUS_PATH_AGENT;
	const char *capability = "";	/* Default is "KeyboardDisplay" */
	int rc;
