...
#include "nodes/vt_interface.h"
#include "nodes/att_interface.h"
...
const char *appd_version = "gatewayd_fxa " BUILD_VERSION_LABEL;

const char *appd_template_version = "ayla_gateway_only_demo_v1.0";
...
#define GET_DEVICE_MAC_ADDRESS	"ifconfig br-lan | awk '/HWaddr/ {print $5}'"
        ...
        size_t len, const struct op_args *args)
{
	...
	if (get_sysinfo_status) {
			...
			prop_send_by_name("device_mac_address");
 ...
 */
static enum err_t appd_device_mac_address_send(struct prop *prop, int req_id,
                   const struct op_options *opts)
{
	FILE *fp;

	fp = popen(GET_DEVICE_MAC_ADDRESS,"r");
	if (fp == NULL) {
		log_err("Get device mac address failed");
		exit(1);
	}
	fscanf(fp, "%s", device_mac_address);
	pclose(fp);
	log_debug(" device_mac_address is :%s\n", device_mac_address);
	return prop_arg_send(prop, req_id, opts);
}
...
*/
		{
			.name = "device_mac_address",
			.type = PROP_STRING,
			.send = appd_device_mac_address_send,
			.arg = &device_mac_address,
			.len = sizeof(device_mac_address),
		},
        {
