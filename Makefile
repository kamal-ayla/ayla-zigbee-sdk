...
# CMake
CMAKE_DEPS_INCLUDE=-DCMAKE_C_FLAGS="-I$(GST_INCLUDE) -I$(GLIB_INCLUDE) -L$(STAGING_DIR)/usr/lib -L$(STAGING_DIR)/root-brcmbca/lib"
CMAKE_DEFS=-DCMAKE_C_COMPILER=$(TOOLCHAIN_DIR)/bin/$(TARGET_CROSS)gcc -DCMAKE_LINKER=$(TOOLCHAIN_DIR)/bin/$(TARGET_CROSS)gcc -DCMAKE_AR=$(TOOLCHAIN_DIR)/bin/$(TARGET_CROSS)ar $(CMAKE_DEPS_INCLUDE)
...
define Build/Compile
	mkdir -p kvsd_apps/kvsd_stream_hls/build && cd kvsd_apps/kvsd_stream_hls/build && cmake $(CMAKE_DEFS) .. && make -j
	mkdir -p kvsd_apps/kvsd_stream_master/build && cd kvsd_apps/kvsd_stream_master/build && cmake $(CMAKE_DEFS) .. && make -j

#	if [ -d kvsd_apps/kvsd_stream_hls/build ]; then \
#	rm -rf kvsd_apps/kvsd_stream_hls/build \
#	fi; \
#	mkdir -p kvsd_apps/kvsd_stream_hls/build && \
#	cd kvsd_apps/kvsd_stream_hls/build && \
#	cmake $(CMAKE_DEFS) .. && \
#	make -j
